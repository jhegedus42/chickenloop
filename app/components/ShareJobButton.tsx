...
export default function ShareJobButton({ jobTitle, shortDescription, url }: ShareJobButtonProps) {
  const [showOptions, setShowOptions] = useState(false);
  const [copied, setCopied] = useState(false);
  const [isMobile, setIsMobile] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);

  // Detect mobile device via userAgent
  useEffect(() => {
    const detectMobile = () => {
  ...
  const handleShare = async () => {
    ...
    if (isMobile && navigator.share && navigator.canShare && navigator.canShare(shareData)) {
      ...
      // Desktop: toggle share options dropdown
  ...
  return (
    <div className="relative inline-block" ref={dropdownRef}>
      <button
        onClick={handleShare}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors"
